...
> implements RepositoryPort<Aggregate>
{
  ...
  async findAllPaginated(
    ...
    const result: Document<unknown>[] = await mongoModel.find({})
    .limit(params.limit)
    .skip(params.offset);
  ...
  async delete(entity: Aggregate): Promise<boolean> {
    entity.validate();
    this.logger.debug(
      `[${RequestContextService.getRequestId()}] deleting entities ${entity.id} from ${this.modelName}`,
    );
    const mongoModel = this.pool.model(this.modelName, this.mongoSchema);
    const result: any = await mongoModel.deleteOne({ _id: entity.id });
    await entity.publishEvents(this.logger, this.eventEmitter);

    return (result as number) > 0;
  }

  async insert(entity: Aggregate | Aggregate[]): Promise<AggregateID | AggregateID[]> {
    ...
    try {
      ...
      this.logger.debug(
        `[${RequestContextService.getRequestId()}] writing ${
          entities.length} entities to "${this.modelName}" model, Ids: ${entityIds}`
      );
      ...
      if (error instanceof MongoError.ValidationError) {
        const subErrors = error.errors ? MongoMapper.validationFormatSubErrors(error.errors) : []
        subErrors.forEach(error => {
          this.logger.debug(
          `[${RequestContextService.getRequestId()}] ${error.field} ::: ${error.message}`,
        );
        })
        throw new UnprocessableEntityException('Record already exists', error, { subErrors });
      }
      throw error;
    }
  }
