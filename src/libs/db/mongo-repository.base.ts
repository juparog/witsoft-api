...
import { AggregateRoot, PaginatedQueryParams, Paginated } from '@witsoft/libs/ddd';
import { Mapper, RepositoryPort } from '@witsoft/libs/ddd';
import { ConflictException } from '@witsoft/libs/exceptions';
import {
  IdentifierSqlToken,
  MixedRow,
...
> implements RepositoryPort<Aggregate>
{
  ...
  protected constructor(
    private readonly _pool: Model<DbModel>,
  ...
  async findOneById(id: string): Promise<Option<Aggregate>> {
    const result: DbModel = await this.pool.findById(id);
    return result ? Some(this.mapper.toDomain(result)) : None;
  }

  async findAll(): Promise<Aggregate[]> {
    const result: DbModel[] = await this.pool.find<DbModel>({});
  ...
  protected get pool(): Model<DbModel> {
    return (
