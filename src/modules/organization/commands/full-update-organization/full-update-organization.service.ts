...
@CommandHandler(FullUpdateOrganizationCommand)
export class FullUpdateOrganizationService implements ICommandHandler {
	constructor(
	...
	async execute(
		command: FullUpdateOrganizationCommand,
		...
		try {
			...
			const result = await this.organizationRepo.transaction(
				async (session) => {
					const result = await this.organizationRepo.findByIdAndReplace(
						command.id,
						updatedOrganization,
						session,
					);
